...
module.exports.get_banners = (req, res) => {
  AdminModel.find().then((banners) => res.json(banners));
};
...
module.exports.edit_banners = (req, res) => {
  ...
  AdminModel.findByIdAndUpdate(DocID, updateData, { new: true }).then(
    (updatedItem) => {
...
module.exports.get_contactus = (req, res) => {
  CustSupportModel.find().then((message) => res.json(message));
};
