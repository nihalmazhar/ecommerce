...
function Counter({userID, productId, itemQuantity}) {
    ...
    const updateQuantity = async(newQuantity) => {
      const response = await axios.patch(`http://localhost:4000/api/cart/${userID}`, {
        productId: productId,
  ...
  return (
    <div><div className='w-24 border-2  border-blue-900 rounded-3xl flex justify-between items-center'><button onClick={counterPlus}  className='text-blue-800 hover:text-[#1e3050]' >
        <FontAwesomeIcon icon={faCirclePlus}  size='2xl' />
    </button>
    {counter} 
    <button onClick={counterMinus} className='text-blue-800 hover:text-[#1e3050]'>
